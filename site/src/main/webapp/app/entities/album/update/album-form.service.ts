...
type AlbumFormGroupContent = {
  ...
  cover: FormControl<IAlbum['cover']>;
  coverContentType: FormControl<IAlbum['coverContentType']>;
  artist: FormControl<IAlbum['artist']>;
  style: FormControl<IAlbum['style']>;
};
...
export class AlbumFormService {
  createAlbumFormGroup(album: AlbumFormGroupInput = { id: null }): AlbumFormGroup {
    ...
    return new FormGroup<AlbumFormGroupContent>({
      ...
      cover: new FormControl(albumRawValue.cover, {
        validators: [Validators.required],
      }),
      coverContentType: new FormControl(albumRawValue.coverContentType),
      artist: new FormControl(albumRawValue.artist),
      style: new FormControl(albumRawValue.style),
    });
  }
