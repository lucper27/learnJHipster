...
describe('Album Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    ...
    it('Should call Style query and add missing value', () => {
      const album: IAlbum = { id: 456 };
      const style: IStyle = { id: 87083 };
      album.style = style;

      const styleCollection: IStyle[] = [{ id: 61034 }];
      jest.spyOn(styleService, 'query').mockReturnValue(of(new HttpResponse({ body: styleCollection })));
      const additionalStyles = [style];
      const expectedCollection: IStyle[] = [...additionalStyles, ...styleCollection];
      jest.spyOn(styleService, 'addStyleToCollectionIfMissing').mockReturnValue(expectedCollection);

      activatedRoute.data = of({ album });
      comp.ngOnInit();

      expect(styleService.query).toHaveBeenCalled();
      expect(styleService.addStyleToCollectionIfMissing).toHaveBeenCalledWith(
        styleCollection,
  ...
  describe('Compare relationships', () => {
    ...
    describe('compareStyle', () => {
      it('Should forward to styleService', () => {
        const entity = { id: 123 };
        const entity2 = { id: 456 };
        jest.spyOn(styleService, 'compareStyle');
        comp.compareStyle(entity, entity2);
        expect(styleService.compareStyle).toHaveBeenCalledWith(entity, entity2);
      });
    });
  });
});
