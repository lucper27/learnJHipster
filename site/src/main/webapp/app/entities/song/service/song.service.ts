...
type RestOf<T extends ISong | NewSong> = Omit<T, 'inclusionDate'> & {
...
export class SongService {
  ...
  protected convertDateFromClient<T extends ISong | NewSong | PartialUpdateSong>(song: T): RestOf<T> {
    return {
      ...song,
      inclusionDate: song.inclusionDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restSong: RestSong): ISong {
    return {
      ...restSong,
      inclusionDate: restSong.inclusionDate ? dayjs(restSong.inclusionDate) : undefined,
    };
  }
